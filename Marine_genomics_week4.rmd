...
You can dowload just the R files for the course via wget with the following link
```{html
...
Assign an object using an assignment operator `<-` or  `=`.
```{r , echo=T}
...
See that object by typing the name you assigned it.
```{r , echo=T}
...
```  
Use square brackets `[]` to get parts of vectors.
...
## A few tips for working with objects
...
Here are a few useful functions: install.packages(), library(), data(), str(), dim(), colnames(), rownames()
...
Install the package `ggplot2`. This only has to be done once and we should then comment out the command to install the package with a #.
...
There are many functions in R that allow us to get an idea of what the data looks like. What are it's dimensions (how many rows and columns?) etc?
...
# Rstudio also allows us to View() the data

```

## How to access parts of the data:

We can look at a single column at a time. There are three ways to access this $, [,#], [,"a"]. Each way has it's own advantages. 
```{r, echo=T}
...
> A few useful commands: dataframename[row , col], str(), dim(), nrow(), unique(), length(), rownames(), summary(), min(), max(), mean(), range(), levels(), factor(), as.factor(), class(), ncol(), nrow(), table(), sum(), quantile(), var()
