...
We can get summaries of vectors with the command `summary()`
```{r , echo=T}
...
We can also look at a single column at a time. There are three ways to access this: $, [,#] or [,"a"]. Each way has it's own advantages:
```{r, echo=T}
...
> Reminder: a few useful commands: dataframename[row , col], str(), dim(), nrow(), unique(), length(), rownames(), summary(), min(), max(), mean(), range(), levels(), factor(), as.factor(), class(), ncol(), nrow(), table(), sum(), quantile(), var()
